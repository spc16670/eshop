...
eshopControllers.controller('ControllerLanding', ['$scope','FactoryAuth',
  '$state',function($scope, FactoryAuth, $state) { 
 
  $scope.currentUser = FactoryAuth.authenticate({ 'operation': "initialize" });
  ...
  $scope.$watch(function() {return FactoryAuth.user},function() {
    $scope.currentUser = FactoryAuth.user;
...
eshopControllers.controller('ControllerLogin', ['$scope','FactoryAuth',
  'FactoryAlert','FactoryRequest',function($scope,FactoryAuth
